...
const Navbar = () => {
  ...
  const navItems = [
    { label: t("nav.about", "אודות"), href: "/#intro" },
    { label: t("nav.philosophy", "פילוסופיה"), href: "/#philosophy" },
    { label: t("nav.services", "שירותים"), href: "/#services" },
    { label: t("nav.programs", "תוכניות"), href: "/#programs" },
    { label: t("nav.blog", "כתיבה"), href: "/blog" },
    { label: t("nav.contact", "צור קשר"), href: "/#contact" },
  ];
  ...
  return (
    <nav className={`fixed top-0 right-0 left-0 z-50 transition-all duration-500 ${scrolled ? "bg-background/90 backdrop-blur-md border-b border-border/50" : ""}`}>
      <div className="section-padding flex items-center justify-between h-16 md:h-20">
        <a href="/" className="font-display text-sm tracking-[0.25em] text-foreground">
          {t("nav.brand", "ERAN BERT")}
        </a>
        ...
        <button onClick={() => setMobileOpen(!mobileOpen)} className="md:hidden flex flex-col gap-1.5 p-2" aria-label="תפריט">
