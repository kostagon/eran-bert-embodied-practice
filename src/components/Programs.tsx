...
const Programs = () => {
  ...
  useEffect(() => {
    supabase.from("programs").select("id,title,short_description,image_url,external_url,sort_order")
  ...
  return (
    <section id="programs" className="py-24 md:py-40 section-padding bg-card">
      <ScrollReveal>
        <div className="mb-16 md:mb-24">
          ...
          <h2 className="font-display text-3xl md:text-4xl lg:text-5xl text-foreground">
            {t("programs.title", "תוכניות והכשרות")}
          </h2>
          <p className="font-body text-base text-muted-foreground mt-4 max-w-lg">
            {t("programs.subtitle", "תוכניות נבחרות, כל אחת מעוצבת בקפידה כדי לתת מענה לצורך ייחודי.")}
          </p>
        </div>
      </ScrollReveal>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-12">
        {programs.map((p, i) => {
          const cta = p.external_url ? (
            <a href={p.external_url} target="_blank" rel="noopener noreferrer"
              className="inline-block mt-5 font-body text-xs tracking-widest text-accent hover:underline">
              לפרטים והרשמה →
            </a>
          ) : null;
          return (
